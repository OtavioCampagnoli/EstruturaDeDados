...
public class OrdenacaoSelectionSort {
    ...
    public OrdenacaoSelectionSort() {
        ...
        if (!selectionSort_v01(vetnulo)) {
            System.out.println("O vetor nulo 'vetnulo' ṇo pode ser ordenado em selectionSort_v01.");
        }

        try {
            selectionSort_v02(vetnulo);
        } catch (Exception ex) {
            System.out.println("O vetor nulo 'vetnulo' ṇo pode ser ordenado em selectionSort_v02.\n");
        }
         ...
         * }
         * catch(Exception ex ){
         * System.out.println("vet ṇo pode ser ordenado em selectionSort_v02...\n");
         * }
         */
    ...
    public void selectionSort_v02(double vetor[]) throws Exception {
        if (vetor == null)
            throw new Exception();
        for (int i = 0; i < vetor.length - 1; i++) {
            // determina a posi̤̣o min do menor valor encontrado
            // entre as posi̵̤es i+1 e vetor.length
            int min = i;
            for (int j = i + 1; j < vetor.length; j++) {
                if (vetor[j] < vetor[min])
                    min = j;
